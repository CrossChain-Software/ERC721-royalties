...
contract ERC165 is IERC165 {
    ...
    constructor () internal {
        ...
        _registerInterface(_INTERFACE_ID_ERC165); // this confirms that this is up to standard 721, so 3rd party apps can trust it
    }
...
contract GenArt721Core is CustomERC721Metadata {
    ...
    mapping(uint256 => Project) projects; // { 42: 'Void', 43: 'Oragami Dream' }

    //All financial functions are stripped from struct for visibility
    mapping(uint256 => address) public projectIdToArtistAddress; // { 43: '0x4r3fn45iw542sdfnsdf' }
    mapping(uint256 => string) public projectIdToCurrencySymbol; // { 43: 'ETH' }
    mapping(uint256 => address) public projectIdToCurrencyAddress;
    mapping(uint256 => uint256) public projectIdToPricePerTokenInWei; // { 43: 1 000 000 000 000 000 000 }
    mapping(uint256 => address) public projectIdToAdditionalPayee; // { 43: '0x876534fdsci43fnidfnsdf' }, like a co-artist
    mapping(uint256 => uint256) public projectIdToAdditionalPayeePercentage; // { 43: 20 }
    mapping(uint256 => uint256) public projectIdToSecondaryMarketRoyaltyPercentage; // to the artist (onlyArtist) = { 43: 2.5 }

    address public artblocksAddress; // used when you create a new 721 contract per project
    uint256 public artblocksPercentage = 10; // artblocks royalty- is this from mint or secondary?
    ...
    // lets say they have 5 devs- send the commissions (10%) from artists to this, then split the payments to the ArtBlocks developers
